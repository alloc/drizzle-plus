...
describe('core type contracts', () => {
  test('query arguments, relations, and ordering', () => {
  ...
  test('query results and SQL result types', () => {
    ...
    expectTypeOf<
      QueryToResult<typeof listQuery, { scalar: true }>
  ...
  test('returning and selection types', () => {
    expectTypeOf<
      ReturningResultFields<
        'one',
        typeof schema.user,
  ...
  test('merged find-many arguments', () => {
    type LeftArgs = {
      columns: { id: true }
    ...
    expectTypeOf<MergedArgs['where']>().toEqualTypeOf<
      { id: 1 } | { name: 'Ada' }
    >()
  })
})
